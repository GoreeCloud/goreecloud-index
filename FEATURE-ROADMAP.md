...
- session-scoped source controls for the currently integrated local Applications, Settings, and Contacts providers, with source changes immediately re-running the current query through the same cancellation-aware Flow path;
- a centralized `IndexDevelopmentSourcePolicy` that rejects unknown providers and GoreeCloud Search from the user-selectable Development set, forces `localOnly=true`, and preserves provider authority evidence without granting new authority;
- explicit UI disclosure that source changes apply only to the current Index session, do not grant permission or platform authority, and cannot silently enable Internet/Web Search or any other remote fallback;
...
Remote Search execution remains blocked on accepted runtime Privacy Shield decision acquisition plus approved GoreeCloud Identity requester/service authentication. The current candidate does not enable live remote Search traffic or claim production Search acceptance. Source selections are deliberately not persisted yet; durable provider preferences require an approved persistence/continuity design, including applicable Everkeep behavior, before they can become authoritative durable state.
...
| FR-009 | Add provider enable/disable, permission review, local-only mode, Internet-provider preference, third-party connection/revocation, history controls if history is implemented, and index/cache clearing with understandable local-versus-remote processing disclosure. | Medium | **Partial Development implementation verified.** PR #35 now provides session-scoped enable/disable controls for Applications, Settings, and Contacts; enforced local-only mode; and explicit disclosure that Internet/Web Search is unavailable and cannot silently activate. Controls do not grant missing authority and are not persisted. Permission-review workflows, durable provider preferences, Internet-provider preference, third-party connection/revocation, history controls if applicable, cache/index clearing, Everkeep continuity for durable settings, and remote-provider user enablement remain open. |
...
1. Preserve the verified fail-closed provider, Search-capability, authority, ranking, cancellation, fan-out, incremental-composition, and session-scoped source-control boundaries while continuing development from PR #35.
2. Add permission-review/authority-explanation UX that reports why a selected protected source such as Contacts is unavailable without manufacturing platform decisions or granting authority locally.
3. Define any future durable provider-preference storage only after the applicable Everkeep continuity/recovery and profile/device scoping requirements are explicit; keep current source selections session-scoped until then.
4. Establish an approved GoreeCloud Identity requester/service-authentication path and real Privacy Shield decision-acquisition adapter before enabling live GoreeCloud Search transport or an Internet-provider preference.
5. Measure incremental rendering/provider completion on representative Android devices and add bounded update coalescing only if evidence shows excessive UI churn; never create a second ranking/composition path.
6. Complete Glaze UI V1.4 rendered/native accessibility, localization/RTL, text-scaling, reduced-effects, representative-device, and performance acceptance.
7. Add Files, Calendar, media, and other first-party providers one bounded authority contract at a time, then complete Platform-System runtime acceptance, production signing/provenance, recovery/rollback, Release Candidate, production, and Stable gates without collapsing independent authorities.
